    ...
    return async (req, _, next) => {
        try {
            ...
            if (session.role == 'superAdmin'
                || await session.groups.find((element) => element.group.chats.find((element1) => element1.id == req.params.id) && element.isAdmin) != undefined
                || (await session.groups.find((element) => element.group.chats.find((element1) => element1.id == req.params.id)) != undefined
